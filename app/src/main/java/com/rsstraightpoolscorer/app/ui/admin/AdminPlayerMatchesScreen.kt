    ...
    Column(Modifier.fillMaxSize().padding(16.dp)) {
      ...
      Column(
        ...
        rows.forEach { m ->
          ...
          ElevatedCard(
              modifier =
                  Modifier.fillMaxWidth().clickable { onEditMatch(m.week, m.aRoster, m.bRoster) }
          ) {
            Column(
                    ...
                    text =
                        buildAnnotatedString {
                          if (!haveScore) {
                            ...
                            withStyle(
                                SpanStyle(color = myColor, fontWeight = FontWeight.SemiBold)
                            ) {
                              append(myScore.toString())
                            }
                            append(" - ")
                            withStyle(
                                SpanStyle(color = oppColor, fontWeight = FontWeight.SemiBold)
                            ) {
              ...
              Row(modifier = Modifier.fillMaxWidth()) {
                ...
                Text(
                    text =
                        buildAnnotatedString {
                          withStyle(
                              SpanStyle(color = playedColor, fontWeight = FontWeight.SemiBold)
                          ) {
                            append(playedText)
                          }
                          append("   ")
                          withStyle(
                              SpanStyle(color = countedColor, fontWeight = FontWeight.SemiBold)
                          ) {
                            append(countedWord)
                          }
                        },
